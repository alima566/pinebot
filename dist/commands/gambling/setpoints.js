        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!gamblingChannel) {
                return message.reply({
                    content: "Can't set pina coladas to members as no gambling channel has been set yet."
                });
            }
            ...
            if (mention == "all") {
                ...
                members.forEach((mem) => __awaiter(this, void 0, void 0, function* () {
                    if (!mem.user.bot) {
                        yield gambling_1.setPoints(message.guild.id, mem.id, +points);
                    }
                }));
                const memberCount = members.filter((mem) => !mem.user.bot).size;
                return message.channel.send({
                    content: `You have set **${memberCount}** member${memberCount != 1 ? "s" : ""} pina coladas to \`${utils_1.formatNumber(+points)}\`.`
                });
            }
            if (mention.user.bot) {
                return message.reply({ content: "You can not set bot's pina coladas." });
            }
            const newPoints = yield gambling_1.setPoints(message.guild.id, mention.id, +points);
            return message.channel.send({
                content: `Pina colada's have been set to \`${utils_1.formatNumber(newPoints)}\` for **${mention.user.tag}**.`
            });
        });
    }
};
